...
function renderChart() {
  ...
  // Aggregate once per month before chart interpolation; do not build a date
  // for every SKU/day on each render of the real 99k-record dataset.
  const monthlyTotals = new Map();
  for (const row of state.snapshot.data.monthly || []) {
    const key = JSON.stringify([row.product_id, row.date]);
    const total = monthlyTotals.get(row.date) || { raw: 0, adjusted: 0 };
    total.raw += row.quantity;
    total.adjusted += adjustments.has(key) ? adjustments.get(key) : row.quantity;
    monthlyTotals.set(row.date, total);
  }
  for (const [month, total] of monthlyTotals) {
    ...
    if (end > result.parameters.as_of || end <= start) continue;
    const days = Math.round((monthEnd - new Date(month + "T00:00:00Z")) / 86400000);
    for (let i = 0; i < days; i++) {
      const index = indices.get(shiftDate(month, i));
      if (index === undefined) continue;
      points[index].raw += total.raw / days;
      points[index].adjusted += total.adjusted / days;
    }
  }
...
function calculationTrace(line, product, stock) {
  const a = line.audit;
  if (!a || !a.history.length) return "";
  const shipments = state.snapshot.data.shipments.filter((s) => s.product_id === line.product_id);
  const included = shipments.filter((s) => s.expected_date >= state.result.parameters.as_of && s.expected_date <= line.coverage_end);
  return `<h3 class="detail-subtitle">От исходных продаж к заказу</h3>
    <p class="modal-copy">Код 1С: ${escapeHTML(line.product_id.split(":").slice(1).join(":") || line.product_id)} · ${escapeHTML(a.history[0].date.slice(0, 7))} — ${escapeHTML(a.history[a.history.length - 1].date.slice(0, 7))} · ${a.history_days} дней полных месяцев. Незавершённый месяц не включён.</p>
    <ol class="method-list" id="calculation-trace">
      <li><strong>Исходные продажи:</strong> ${number(a.raw_total, 2)} ед.</li>
      <li><strong>После удаления всплесков:</strong> ${number(a.after_spikes_total, 2)} ед. Медиана/MAD по документам и месяцам; месячные итоги не складываются с документами.</li>
      <li><strong>После компенсации вероятного отсутствия:</strong> ${number(a.after_stockouts_total, 2)} ед. Нулевой остаток и низкие продажи сопоставляются с нормальными месяцами.</li>
      <li><strong>Сезонность и тренд:</strong> после снятия исторической сезонности ${number(a.base_daily_demand, 4)} ед./день × тренд ${number(line.trend_factor, 3)} = ${number(a.trend_daily_demand, 4)}. Будущая сезонность применяется по календарным дням; средний коэффициент горизонта ${number(line.seasonality_factor, 3)}.</li>
      <li><strong>Прогноз:</strong> ${number(line.forecast_demand, 2)} за ${line.coverage_days - state.result.parameters.safety_stock_days} дней + страховой запас ${number(line.safety_stock, 2)} за ${state.result.parameters.safety_stock_days} дней. Округлённый целевой запас: ${number(line.target_stock)}.</li>
      <li><strong>Остаток:</strong> ${number(stock.on_hand)} − резерв ${number(stock.reserved)} = ${number(line.available_stock)}. Дата среза: ${product.stock_date ? date(product.stock_date) : "не указана"}.</li>
      <li><strong>В пути в пределах горизонта:</strong> ${number(line.incoming_quantity)}.${included.length ? `<ul>${included.map((s) => `<li>${date(s.expected_date)}: ${number(s.quantity)} ед.</li>`).join("")}</ul>` : " Подходящих поставок нет."}</li>
      <li><strong>Чистая потребность:</strong> max(0, ${number(line.target_stock)} − ${number(line.available_stock)} − ${number(line.incoming_quantity)}) = ${number(line.net_requirement)}.</li>
      <li><strong>MOQ и кратность:</strong> минимум ${number(product.min_order_quantity)}, кратность ${number(product.pack_size)}. При положительной потребности применяем минимум и округляем вверх: <strong>${number(line.order_quantity)} ед.</strong></li>
      <li><strong>Решение менеджера:</strong> проверить остаток и допущения, затем экспортировать подготовленный заказ. Поставщику ничего не отправляется.</li>
    </ol>
    <details id="history-trace"><summary>Помесячные данные и корректировки</summary><div class="table-scroll"><table>
      <thead><tr><th>Месяц</th><th>Продажи</th><th>После всплесков</th><th>После компенсации</th><th>Остаток месяца</th><th>Сезонность</th></tr></thead>
      <tbody>${a.history.map((h) => `<tr><td>${escapeHTML(h.date.slice(0, 7))}</td><td>${number(h.raw, 2)}</td><td>${number(h.after_spikes, 2)}</td><td>${number(h.after_stockouts, 2)}</td><td>${h.stock === null ? "нет данных" : number(h.stock, 2)}</td><td>${number(h.seasonality, 3)}</td></tr>`).join("")}</tbody>
    </table></div></details>`;
}

async function realExample() {
  if (state.busy) return;
  const id = "iek:010400929_";
  if (!state.snapshot?.data.products.some((p) => p.id === id) || !state.snapshot.data.monthly?.length) {
    const response = await api("excel-dataset");
    importPreview(await response.json(), "Реальные Excel: загрузите данные, затем нажмите «Пример расчёта»");
    return;
  }
  state.params = { as_of: "2026-09-23", lookback_days: 365, review_period_days: 14, safety_stock_days: 7 };
  await calculate();
  if (!state.result) return;
  state.view = "orders";
  state.filter = "all";
  state.supplier = "";
  state.search = "ivr21-1-25";
  $("#search").value = "IVR21-1-25";
  render();
  showDetail(id);
}
...
function importPreview(data, name, demo = false) {
  ...
  openModal(
    ...
    `<p class="modal-copy">${demo ? "Учебные данные: кабели, автоматика и освещение. История за последние 30 дней содержит три разовых всплеска." : `Файл: <strong>${escapeHTML(name)}</strong>. Проверьте состав данных перед загрузкой.`}</p><div class="import-summary">${[
      ...
      ["Продаж / месяцев", data.sales.length + (data.monthly?.length || 0)],
...
async function confirmImport() {
  ...
  try {
    ...
    state.etag = response.headers.get("ETag");
    state.result = null;
    if (pending.data.source_date) {
      state.params.as_of = shiftDate(pending.data.source_date, 1);
      state.params.lookback_days = 365;
    }
    if (pending.demo)
      state.params = {
...
document.addEventListener("click", async (event) => {
  ...
  try {
    ...
    if (action === "real-example") await realExample();
    if (action === "excel") {

...
type Line struct {
	...
	ForecastDemand    float64      `json:"forecast_daily_demand"`
...
func Calculate(ctx context.Context, d Dataset, r Request) (Result, error) {
	...
	for _, p := range products {
		...
		line.ForecastDemand = line.DailyDemand
		if len(supplier.Seasonality) > 0 {
			factorSum := 0.0
			for day := 0; day < coverage; day++ {
				factorSum += supplier.Seasonality[int(asOf.AddDate(0, 0, day).Month())-1]
			}
			line.ForecastDemand = deseasonalized / denominator * factorSum / float64(coverage)
			if line.DailyDemand > 0 {
				line.SeasonalFactor = line.ForecastDemand / line.DailyDemand
			}
		}
		if adjusted == 0 {
		...
		if float64(line.AvailableStock+arrivingDuringLead) < line.ForecastDemand*float64(supplier.LeadTimeDays) {
		...
		if len(supplier.Seasonality) > 0 {
			line.TargetStock = math.Ceil(line.ForecastDemand*float64(coverage) - 1e-9)
		}
